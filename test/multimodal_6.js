...
// TTS (朗讀)
// =====================
async function speakTextWithGeminiFirst(text, options={}){
  const speakText = String(text || '').trim();
  if(!speakText) return false;
  try{
    if(window.parent && window.parent !== window && typeof window.parent.playGeminiNativeSpeech === 'function'){
      await window.parent.playGeminiNativeSpeech(speakText, options);
      return true;
    }
  }catch(err){
    console.warn('Parent Gemini speech failed, fallback to local audio.', err);
  }
  try{
    ...
    console.warn('Local Gemini TTS failed, fallback to browser speech.', err);
  }
  try{
...
function speakPoem(poem){
  if(!poem) return;
  speakTextWithGeminiFirst(`${poem.name}。作者${poem.author}。${poem.content}`, {
    bubbleText:'我示範朗讀給你聽。',
    voiceName:'Leda'
  });
}
