...
(function(){
  ...
  window.askGeminiFromInput = async function(){
    ...
    try{
      ...
      if(poemsLoaded){
        await handleSentence(question);
        const replyText = ($('qaReply')?.textContent || '').trim() || '我先幫你找到一個答案。';
        pushFrontConversationLog('assistant', replyText);
        if(shouldSpeak) await speakTextWithGeminiFirst(replyText, { bubbleText:'我來回答你。', voiceName:'Leda' });
        return;
      }
      if(!geminiAvailable){
        const msg = '請先載入詩詞資料，或在伺服器設定 GEMINI_API_KEY 後再試一次。';
        setReply(msg);
      ...
      const msg = '回答失敗：' + (err?.message || err);
      setReply(msg);
      pushFrontConversationLog('assistant', msg);
    }
  };
  ...
  window.initStt = function(){
    ...
    rec.onerror = e => {
      ...
      if (err === 'no-speech') { $('sttStatus').textContent = '我剛剛沒有聽到聲音，請再說一次。'; return; }
      $('sttStatus').textContent = '語音辨識暫時失敗：' + err;
    };
    rec.onresult = ev => {
      let interim = '', fin = '';
      for(let i = ev.resultIndex; i < ev.results.length; i++){
  ...
  function createVoiceChatRecognizer(){
    ...
    rec.onerror = e => {
      const err = e?.error || 'unknown';
      if (err === 'no-speech') {
        setVoiceChatStatus('我還沒聽到聲音，再說一次喔。', 'var(--gold-light)');
        if (voiceChatActive && !voiceChatAwaiting && !ttsSpeaking) {
          setTimeout(() => { try { rec.start(); } catch(_){} }, 240);
        }
        return;
      }
      setVoiceChatStatus('語音辨識暫時失敗：' + err, '#e05050');
    };
    rec.onresult = async ev => {
      ...
      if(fin.trim() && !voiceChatBusy){
        ...
        try{
          ...
          const msg = /quota|resource_exhausted|429/i.test(rawMsg)
            ? 'Gemini 聲音配額已滿，新的語音暫時無法即時產生，已改用備援聲音。'
            : 'Gemini 語音對話失敗：' + rawMsg;
          setReply(msg);
          pushFrontConversationLog('assistant', msg);
...
function speakShortText(text){
  if(!text) return;
  speakTextWithGeminiFirst(String(text), { bubbleText:'我說給你聽。', voiceName:'Leda' });
}
...
function readQuizQuestion(){
  ...
  window.setCurrentPoemFromBridge = function(poemLike){
    if(!poemLike){ setCurrentPoem(null); return null; }
    const name = poemLike.name || poemLike.poem_name || '';
    const author = poemLike.author || poemLike.poet_name || '';
    let match = null;
    if(name && author){
      match = poemsDB.find(p => normalize(p.name) === normalize(name) && normalize(p.author) === normalize(author)) || null;
    }
    if(!match && name){
      match = poemsDB.find(p => normalize(p.name) === normalize(name)) || null;
      if(!match){
        const found = findByTitle(name);
        if(found.length) match = found[0];
      }
    }
    if(!match && author){
      const found = findByAuthor(author);
      if(found.length === 1) match = found[0];
    }
    setCurrentPoem(match || poemLike || null);
    return bridgePoem(currentPoem);
  };
  window.clearCurrentPoemFromBridge = function(){ setCurrentPoem(null); return null; };
