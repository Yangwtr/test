    ...
    function playAudioBase64(audioBase64, mimeType='audio/wav') {
      const player = $('audio-native-tts');
    ...
    async function sendAskQuestion(extraPrompt='') {
      ...
      await withEngine(async (eng, doc) => {
        ...
        if (input) input.value = extraPrompt ? `${question}
${extraPrompt}`.trim() : question;
    ...
    function initEvents() {
      ...
      $('btn-global-read-aloud').onclick = readCurrentAnswer;
      ...
      $('btn-ask-reset').onclick = () => {
        ...
        withEngine((eng) => eng.clearAskSession && eng.clearAskSession());
      };
      $('btn-ask-read-aloud').onclick = readCurrentAnswer;
      $('input-ask-question').addEventListener('keydown', e => {
        if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') sendAskQuestion();
      });
      $('btn-ask-voice-start').onclick = () => withEngine((eng) => { eng.startQaStt && eng.startQaStt(); setMode('ask'); });
      ...
      $('btn-quiz-read-question').onclick = () => withEngine((eng) => eng.readQuizQuestion ? eng.readQuizQuestion() : speakText($('text-quiz-question').textContent));
