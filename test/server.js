...
async function generateChatResponse(contextPayload, { concise = false, voiceChat = false } = {}) {
  ...
  const parsed = parseModelJsonResponse(response.text) || { answer: response.text || '模型沒有回傳內容。' };
  return {
    model: MODEL,
...
async function generateTtsPayload(text, { voiceName, stylePrompt } = {}) {
  ...
  const work = (async () => {
    ...
    let inlineAudio = extractInlineAudioPart(response);
    if (!inlineAudio?.data) {
      ...
      if (shortened && shortened !== cleanText) {
        const retryResponse = await ai.models.generateContent({
          ...
          config: { responseModalities: ['AUDIO'], speechConfig: { voiceConfig: { prebuiltVoiceConfig: { voiceName: finalVoice } } } },
        });
        inlineAudio = extractInlineAudioPart(retryResponse);
      }
    }
    if (!inlineAudio?.data) {
...
app.post('/api/gemini/tts', async (req, res) => {
  try {
    if (!ai) {
      return res.json({ ...makeEmptyTtsPayload(), error: '伺服器尚未設定 GEMINI_API_KEY。' });
    }
